...
# 从本地读入训练与测试数据集。
train = pd.read_csv('../Dataset/IMDB/labeledTrainData.tsv', delimiter='\t')
test = pd.read_csv('../Dataset/IMDB/testData.tsv', delimiter='\t')
...
# 结果输出到本地硬盘。
submission_count.to_csv('../Dataset/IMDB/submission_count.csv', index=False)
submission_tfidf.to_csv('../Dataset/IMDB/submission_tfidf.csv', index=False)

# 从本地读入未标记数据。
unlabeled_train = pd.read_csv('../Dataset/IMDB/unlabeledTrainData.tsv', delimiter='\t', quoting=3)
...
model_name = "../Dataset/IMDB/300features_20minwords_10context"
...
model = Word2Vec.load("../Dataset/IMDB/300features_20minwords_10context")
...
output.to_csv("../Dataset/IMDB/submission_w2v.csv", index=False, quoting=3)
